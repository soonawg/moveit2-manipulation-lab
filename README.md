...
- `moveit2_manipulation/`
  - `launch/` : RViz / spawn / simulation launch files  
    ...
    - `spawn_robot.launch.py`, `spawn_robot_w_controller.launch.py`, `start_simulation.launch.py` :contentReference[oaicite:4]{index=4} :contentReference[oaicite:5]{index=5} :contentReference[oaicite:6]{index=6}  
  - `urdf/` : robot models (Xacro)
    - `cobot.urdf.xacro`, `cube_pick_place.urdf.xacro` :contentReference[oaicite:7]{index=7} :contentReference[oaicite:8]{index=8}  
  - `config/` : controllers / RViz config
    - `joints_controllers.yaml`, `arm.rviz` :contentReference[oaicite:9]{index=9} :contentReference[oaicite:10]{index=10}  
  - `my_manipulation_robot/` : small scripts for joint motion and testing
    - `joint_move.py` and other utilities :contentReference[oaicite:11]{index=11}  
